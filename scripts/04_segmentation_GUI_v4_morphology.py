...
def postprocess_mask(
    ...
    """
    Postprocesamiento morfologico comun.

    Orden usado:
    1. recorte a ROI;
    2. apertura morfologica para cortar puentes finos;
    3. conservar componente mas grande;
    4. cierre morfologico suave para regularizar bordes;
    5. relleno de huecos.

    La apertura y el cierre son configurables desde la GUI.
    Opciones disponibles para ambos: 3x3, 5x5, 9x9, 13x13 y 17x17.
    """
    ...
    # 1. Apertura primero: corta puentes finos antes de seleccionar componente.
    ...
    # 2. Conservar solo la componente mas grande dentro de la ROI.
    clean = keep_largest_component(clean)
    clean &= roi_mask

    # 3. Cierre suave para regularizar bordes despues de separar componentes.
    clean_u8 = (clean.astype(np.uint8)) * 255
...
def segment_region_growing(
    ...
    return postprocess_mask(mask, roi_rect=roi_rect, open_kernel_size=open_kernel_size, close_kernel_size=close_kernel_size)
...
def segment_local_threshold(
    ...
    return postprocess_mask(full_mask, roi_rect=roi_rect, open_kernel_size=open_kernel_size, close_kernel_size=close_kernel_size)
...
class SegmentationGUI(QMainWindow):
        ...
        try:
            if method == "Region growing":
                ...
                parameters = {
                    ...
                    "postprocessing": "ROI + opening configurable + largest component + closing configurable + fill holes",
                }
            elif method == "Umbral local":
    ...
    def display_all(self) -> None:
        ...
        self.main_image_label.set_image(make_main_overlay(display_image, self.predicted_mask, self.roi_rect, self.seed_xy))
        ...
        self.comparison_label.set_image(make_comparison_overlay(display_image, self.predicted_mask, self.manual_mask))
    ...
    def save_current_result(self) -> None:
        ...
        overlay = make_comparison_overlay(overlay_base, self.predicted_mask, self.manual_mask)
